...
Output: mul_vectors.txt (15000 dong, xem breakdown theo category o gen_all())
...
def gen_all(
    seed=42,
    n_random=12175,     # random_normal x random_normal
    n_denorm=500,        # so denormal
    n_edge=300,          # overflow_edge, underflow_edge (moi loai)
    n_specialmix=100,    # moi special x random, ca 2 chieu (A/B)
    n_identity=300,      # x(+1.0) va x(-1.0), moi loai
    n_boundary=100,      # sat nguong overflow (exponent_temp == 254)
):
    """
    Tong so vector mac dinh = 15000, chia theo category nhu sau:
        random_normal      : n_random        = 12175
        special_*          : 5*5             =    25   (co dinh)
        specialmix_*       : n_specialmix*10 =  1000
        denormal           : n_denorm        =   500
        overflow_edge      : n_edge          =   300
        underflow_edge     : n_edge          =   300
        identity_mul1      : n_identity      =   300
        identity_mulneg1   : n_identity      =   300
        boundary_exp254    : n_boundary      =   100
        -----------------------------------------------
        TONG                                 = 15000

    Cac tham so co the chinh lai neu muon doi ty le / tong so vector khac.
    """
    ...
    # 8) Boundary exponent product == 254 (sat overflow)
    #    exp_A + exp_B - 127 == 254  <=>  exp_A + exp_B == 381
    #    Voi exp_A trong [127,254] thi exp_B = 381 - exp_A luon nam trong
    #    [127,254] -> luon hop le, khong bi loc bo vector nao.
...
# =================================================================
# GHI CHU VE HANH VI DENORMAL CUA RTL (khong phai bug cua script nay)
# =================================================================
# fp_mul_mantissa.v luon lam {1'b1, fraction} cho ca A va B, tuc la gia
# dinh hidden-bit = 1 ngay ca khi exponent = 0 (denormal / zero). Voi input
# = 0 thi khong sao vi da co nhanh special-case A_zero/B_zero chan truoc.
# Nhung voi denormal khac 0 (exponent=0, fraction!=0), RTL hien tai se NHAN
# SAI ve mat toan hoc (theo dung chuan IEEE-754 thi denormal phai la
# 0.fraction x 2^(-126), khong phai 1.fraction x 2^(-127)).
# => Day la diem dang de dua vao phan "Limitations" / "Future work" cua bai
#    bao (muc 8. Discussion trong PAPER_ROADMAP.docx), va la mot novelty co
#    the them: RTL A ho tro dung denormal, RTL B (hien tai) thi khong.